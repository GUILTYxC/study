...
from myapp.models import Cet6_year, cet6data,Stardict,Wordbook
from django.shortcuts import redirect, reverse
from django.views.decorators.http import require_POST
from myadmin.models import User

def zhuci(request):
    years = Cet6_year.objects.all().order_by('name')
    context = {'years': years}
    if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest' and request.method == "POST":
       # 处理AJAX POST请求
        selected_year = request.POST.get('selectedYear', None)
        selected_type = request.POST.get('selectedType', None)
        results = cet6data.objects.filter(year=selected_year, section_type=selected_type)
    ...
    return render(request, 'myapp/zhuci6.html', context)
